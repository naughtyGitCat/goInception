...
func (s *testParserSuite) TestDDL(c *C) {
	...
	table := []testCase{
		...
		// OceanBase 4.x table options (oceanbase/Integration#95c8f822)
		// 纯解析测试 (期望输出留空，只验证 parse 成功)
		{"CREATE TABLE test_tbl1 (col1 INT, col2 TIMESTAMP) DYNAMIC_PARTITION_POLICY(ENABLE = true, TIME_UNIT = 'hour', PRECREATE_TIME = '3 hour', EXPIRE_TIME = '1 day', TIME_ZONE = '+8:00', BIGINT_PRECISION = 'none') PARTITION BY RANGE (col2) (PARTITION P0 VALUES LESS THAN (TIMESTAMP '2024-11-11 13:30:00'))", true, ""},
		{"CREATE TABLE t2 (a INT) DYNAMIC_PARTITION_POLICY = (ENABLE = false, TIME_UNIT = 'day') PARTITION BY RANGE (a) (PARTITION p0 VALUES LESS THAN (10))", true, ""},
		{"create table t (c int primary key) organization index default charset=utf8mb4 row_format=dynamic compression='lz4_1.0' replica_num=1 block_size=16384 use_bloom_filter=false enable_macro_block_bloom_filter=true tablet_size=134217728 pctfree=0 merge_engine=delete_insert skip_index_level=0 partition by hash(c) (partition p0, partition p1) with column group(all columns, each column);", true, ""},
		{"create table t1 (c1 int, c2 int) organization heap default charset=utf8mb4 row_format=dynamic compression='lz4_1.0' replica_num=1 block_size=16384 use_bloom_filter=false enable_macro_block_bloom_filter=false tablet_size=134217728 pctfree=0 merge_engine=delete_insert skip_index_level=1 delta_format='encoding' with column group(all columns, each column);", true, ""},
...
ENGINE=INNODB PARTITION BY LINEAR HASH (a) PARTITIONS 1;`, true, "CREATE TABLE `t1` (`a` INT,`b` TIMESTAMP DEFAULT '0000-00-00 00:00:00') ENGINE = INNODB PARTITION BY LINEAR HASH (`a`) PARTITIONS 1"},
		...
		// Coverage for scenarios listed in goInception支持ob一级和二级分区创建报告.md
		// 注：所有用 SUBPARTITION TEMPLATE (...) 的用例都被刻意省略，因为本仓
		// parser.y 不支持 TEMPLATE 语法（yacc shift/reduce 冲突），详见
		// docs/naughtygitcat-changelog.md。同样跳过来自 4183b18b 的"PARTITIONS
		// after SUBPARTITION TEMPLATE"系列。
		{"CREATE TABLE t2_m_rch(col1 INT,col2 INT) PARTITION BY RANGE COLUMNS(col1) SUBPARTITION BY HASH(col2) SUBPARTITIONS 5 (PARTITION p0 VALUES LESS THAN(100), PARTITION p1 VALUES LESS THAN(200), PARTITION p2 VALUES LESS THAN(300))", true, ""},
		{"CREATE TABLE t2_m_rck(col1 INT,col2 INT) PARTITION BY RANGE COLUMNS(col1) SUBPARTITION BY KEY(col2) SUBPARTITIONS 3 (PARTITION p0 VALUES LESS THAN(100), PARTITION p1 VALUES LESS THAN(200), PARTITION p2 VALUES LESS THAN(300))", true, ""},
		{"CREATE TABLE t2_m_lch(col1 INT,col2 INT) PARTITION BY LIST COLUMNS(col1) SUBPARTITION BY HASH(col2) SUBPARTITIONS 5 (PARTITION p0 VALUES IN(100), PARTITION p1 VALUES IN(200), PARTITION p2 VALUES IN(300))", true, ""},
		{"CREATE TABLE t2_m_lck(col1 INT,col2 INT) PARTITION BY LIST COLUMNS(col1) SUBPARTITION BY KEY(col2) SUBPARTITIONS 3 (PARTITION p0 VALUES IN(100), PARTITION p1 VALUES IN(200), PARTITION p2 VALUES IN(300))", true, ""},
		{"CREATE TABLE t2_f_rr(col1 INT,col2 TIMESTAMP) PARTITION BY RANGE(col1) SUBPARTITION BY RANGE(UNIX_TIMESTAMP(col2)) (PARTITION p0 VALUES LESS THAN(100) (SUBPARTITION sp0 VALUES LESS THAN(UNIX_TIMESTAMP('2021/04/01')), SUBPARTITION sp1 VALUES LESS THAN(UNIX_TIMESTAMP('2021/07/01')), SUBPARTITION sp2 VALUES LESS THAN(UNIX_TIMESTAMP('2021/10/01')), SUBPARTITION sp3 VALUES LESS THAN(UNIX_TIMESTAMP('2022/01/01'))), PARTITION p1 VALUES LESS THAN(200) (SUBPARTITION sp4 VALUES LESS THAN(UNIX_TIMESTAMP('2021/04/01')), SUBPARTITION sp5 VALUES LESS THAN(UNIX_TIMESTAMP('2021/07/01')), SUBPARTITION sp6 VALUES LESS THAN(UNIX_TIMESTAMP('2021/10/01')), SUBPARTITION sp7 VALUES LESS THAN(UNIX_TIMESTAMP('2022/01/01'))))", true, ""},
		{"CREATE TABLE t2_f_rch (col1 INT,col2 INT) PARTITION BY RANGE(col1) SUBPARTITION BY HASH(col2) (PARTITION p0 VALUES LESS THAN(100) (SUBPARTITION sp0, SUBPARTITION sp1, SUBPARTITION sp2), PARTITION p1 VALUES LESS THAN(200) (SUBPARTITION sp3, SUBPARTITION sp4, SUBPARTITION sp5))", true, ""},
		{"CREATE TABLE t2_f_hl (col1 INT,col2 INT) PARTITION BY HASH(col1) SUBPARTITION BY LIST(col2) (PARTITION p1 (SUBPARTITION sp0 VALUES IN (2020), SUBPARTITION sp1 VALUES IN (2021), SUBPARTITION sp2 VALUES IN (2022), SUBPARTITION sp3 VALUES IN (2023)), PARTITION p2 (SUBPARTITION sp4 VALUES IN (2020), SUBPARTITION sp5 VALUES IN (2021), SUBPARTITION sp6 VALUES IN (2022), SUBPARTITION sp7 VALUES IN (2023)))", true, ""},
		{"CREATE TABLE t2_f_hk (col1 INT,col2 INT) PARTITION BY HASH(col1) SUBPARTITION BY KEY(col2) (PARTITION p1 (SUBPARTITION sp0, SUBPARTITION sp1, SUBPARTITION sp2, SUBPARTITION sp3), PARTITION p2 (SUBPARTITION sp4, SUBPARTITION sp5, SUBPARTITION sp6, SUBPARTITION sp7))", true, ""},
	}
	s.RunTest(c, table, false)
...
func (s *testParserSuite) TestAsOfClause(c *C) {
	table := []testCase{
		...
		{"select * from `t` as of snapshot '1597306800000000000'", true, "SELECT * FROM `t` AS OF SNAPSHOT _UTF8MB4'1597306800000000000'"},
		{"SELECT * FROM `t` AS OF SNAPSHOT TIME_TO_USEC('2026-04-29 13:55:35') * 1000;", true, "SELECT * FROM `t` AS OF SNAPSHOT TIME_TO_USEC('2026-04-29 13:55:35') * 1000"},
